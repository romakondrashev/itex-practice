...
if (isset($limit)) {
    $post_count = $limit;
    $sqlSelect = $dbh->prepare("SELECT queues.ID,`title`,`description`,`author_FID`,`discipline`,`place`,`date`,`is_active` FROM `queues`,`queue_user` WHERE queues.ID = queue_user.FID_queue and queue_user.FID_user = :myID AND queues.author_FID != :myID LIMIT ".$limit);
} else {
    $post_count = -1;
    $sqlSelect = $dbh->prepare("SELECT queues.ID,`title`,`description`,`author_FID`,`discipline`,`place`,`date`,`is_active` FROM `queues`,`queue_user` WHERE queues.ID = queue_user.FID_queue and queue_user.FID_user = :myID  AND queues.author_FID != :myID ");
}
...
if (!empty($queues_info)) :
    ...
    foreach ($queues_info as $index => $queue_info) :
        ...
        $sqlSelectCount = $dbh->query("SELECT COUNT(*) FROM `queue_user` WHERE `FID_queue` = ".$queue_info['ID']." AND `queue_status` = 1");
        ...
        <div class="col-xl-4 col-md-6">
            <div class="card bg-light text-dark mb-4">
                <div class="card-body">
                    ...
                    <div class="card-body-item">
                        <p>Статус очереди:</p>
                        <?php echo $queue_info['is_active'] === '1' ? '<p class="text-success">Активна</p>' : '<p class="text-danger">Не активна</p>' ?>
                    </div>
                    <hr>
